			...
			<tr> 
				...
				<th>  Rp.  <?php  if (!isset($_GET['tarif_pengiriman']) AND !isset($_GET['id_daftar_pengiriman'])){ ?>
                        ...
                        <?php } 
                         ...
                         echo $_SESSION["ongkir"][$nop];?>
                        <?php } ?>
				<td>  <a href ="daftar_pengiriman_ubah.php?nm_kota=<?php echo $pecah['nm_kota']?>&no=<?php echo $nop?>" class="btn btn- btn-light"> UBAH</a> </td>
			</tr>
			</tr>
			<tr> 
...
if (isset ( $_POST['checkout']))
{
    ...
    $koneksi->query("INSERT INTO faktur_beli (id_faktur_beli, id_jasa_pembayaran, id_pembeli, id_status_pembayaran, tot_barang, tot_bayar, qty, tgl, no_pembayaran, no_rekening) VALUES ('$id_faktur_beli', '$_POST[id_jasa_pembayaran]', '$id_pembeli', '$id_status_pembayaran', '$totalbarang', '$totalkeseluruhan','$totalbarang','$tanggal', '$no_pembayaran', '$no_rekening')");
    ...
    foreach ($_SESSION["keranjang"] as $id_barang=>$qty)
    {
		...
		$tarifff=[]; $nof=0; $nor=0;
		$id_status_pengiriman="STATPENG005";//Belum Bayar ("I8B2")
		$id_status_transfer_penjual="018C2";
		$transfer_penjual="0";
		...
		foreach ($_SESSION["keranjang"] as $id_barang=>$qtyyy)
			{
			 ...
			 $subtotal_barang=$qtyyy*$harga_barang; $penilaian="-"; $rating="0";
			 $tarifff[]=$_SESSION["tarif"][$nor];


			$koneksi->query("INSERT INTO faktur_rinci (id_faktur_rinci, id_faktur_beli, id_status_pengiriman, tgl_rinci, id_daftar_pengiriman, id_status_transfer_penjual, id_penjual, transfer_uang_penjual, qty_barang_per_toko, total_bayar, penilaian, rating, komplain)
			VALUES ('$nota_faktur[$nor]','$nota_beli','$id_status_pengiriman', '$tanggal','$tarifff[$nor]', '$id_status_transfer_penjual', '$id_penjual', '$transfer_penjual', '$qtyyy', '$subtotal_barang', '$penilaian','$rating','$komplain')");
			$nor++;
			}
			...
			//UPDATE STOK
			$koneksi->query("UPDATE barang SET stok=stok-$qtyyz WHERE id_barang='$id_barang'");
			$nof++;
			}
